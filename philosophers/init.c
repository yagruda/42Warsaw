...
/*   Updated: 2025/09/23 14:42:50 by yhruda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philosophers.h"

int	init_all(t_philo **philosophers, t_table **table, t_fork **forks,
		int input_num_of_philo)
{
...
// i do spoon_id + 1 and phil_id + 1 to start from 1.
// Accordingly, imagine as 	-1 position in array.

void	init_forks_filos(t_philo *philosophers, t_fork *forks, int n_philos,
		t_table *table)
{
	int	i;
...
void	final_init_table(t_table *table, int argc, char **argv)
{
	...
	create_philo_threads(table->philo);
}
...
void	create_philo_threads(t_philo *philosophers)
{
	...
	{
		if (pthread_create(&philosophers[i].thread_handle, NULL, philosopher_fn,
				&philosophers[i]) != 0)
		{
